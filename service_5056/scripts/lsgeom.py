...
# -- where the exported program lives --------------------------------------

PROGRAM_PREFIX = "DISTI"          # marks the program as generated by this service


def program_name(session):
    """Program name for a session's export. Unique per run, Fanuc-safe.

    The controller refuses a load when the name inside the file differs from the file
    it arrives as, so this drives BOTH the /PROG field and the file name.

    Unique per run on purpose: a fixed name would make every export overwrite the
    previous one on the pendant. Seconds are kept rather than minutes because two
    sessions can be created within the same minute, and that collision would silently
    replace the earlier program.

    Length: the longest name in production is TORXL_NEW_PROG2_5 (17 chars), so 17 is
    proven to work on this controller; the timestamped form is exactly that long."""
    m = re.search(r'(\d{8})_(\d{6})', session)                    # run_20260805_115950
    if m:
        return f"{PROGRAM_PREFIX}_{m.group(1)[4:]}_{m.group(2)}"  # DISTI_0805_115950
    s = re.sub(r'[^A-Za-z0-9]+', '_', session).upper().strip('_')
    return f"{PROGRAM_PREFIX}_{s}"[:17]


def export_path(session_dir):
    """Path to the exported program of a session, or None.

    The file name is no longer fixed - it carries the program name so the two match -
    so nothing may hardcode it. step05 records the name it used; the directory scan is
    only a fallback for sessions written before that."""
    import json as _json
    j = os.path.join(session_dir, 'step05_result.json')
    if os.path.exists(j):
        try:
            with open(j, encoding='utf-8') as f:
                name = _json.load(f).get('current_ls_file')
            if name and os.path.exists(os.path.join(session_dir, name)):
                return os.path.join(session_dir, name)
        except Exception:
            pass
    if not os.path.isdir(session_dir):
        return None
    found = [f for f in os.listdir(session_dir) if f.lower().endswith('.ls')]
    return os.path.join(session_dir, found[0]) if len(found) == 1 else None
