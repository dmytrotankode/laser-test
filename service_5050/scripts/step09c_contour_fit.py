...
def find_2d_transform(target_mask, proj_mask, cam_name):
    ...
    if cam_name in ['back', 'left']:
        ...
        cutoff = None # 100% top circle as requested
...
def generate_overlap_images(target_masks, proj_masks, transforms, out_dir):
    ...
    for cam_name in target_masks:
        ...
        # Apply the exact same cutoff as used in math
        cutoff = None
        if cam_name in ['back', 'left']:
            cutoff = int(h * 0.75)
        ...
        if cutoff:
            ...
            proj_mask = proj_mask.copy()
            target_mask[cutoff:, :] = 0
            proj_mask[cutoff:, :] = 0
            
        # Extract thin outlines instead of solid blobs
        edges_target = cv2.Canny(target_mask, 100, 200) > 0
        edges_proj = cv2.Canny(proj_mask, 100, 200) > 0
        ...
        overlap[edges_target] = c_target
        overlap[edges_proj] = c_proj
        overlap[edges_target & edges_proj] = c_both
        
        cv2.putText(overlap, f"CONTOUR MATH (Upper 75%)", (50, 100), cv2.FONT_HERSHEY_SIMPLEX, 2, (255, 255, 255), 4)
        cv2.putText(overlap, "BLUE: Current Photo Contour", (50, 180), cv2.FONT_HERSHEY_SIMPLEX, 2, c_target, 4)
        cv2.putText(overlap, "ORANGE: Etalon 3D Contour", (50, 260), cv2.FONT_HERSHEY_SIMPLEX, 2, c_proj, 4)
...
def main():
    ...
    logger = PipelineLogger(session_id, base_dir, "STEP 09C: 2D CURRENT POSE FIT (CONTOURS)")
    ...
    out_file = os.path.join(session_dir, 'step09c_current_pose_fit.json')
    res_data = {
    ...
    overlap_dir = os.path.join(session_dir, 'step09c_current_overlap')
